...
import { WaveShieldDefaultError } from "../models/errors/waveshielddefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | WaveShieldDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | WaveShieldDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new WaveShieldDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new WaveShieldDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
